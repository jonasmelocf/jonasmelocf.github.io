...
<template>
	<Button ref="progress-buttons" :disabled="progress?.puzzleState === 'locked'" :title="puzzle.id" :class="['size-10 *:p-0.5 *:mx-auto rounded-full p-0 text-(--vp-c-white)!', {
		'bg-(--vp-c-success-3)': progress?.puzzleState === 'done',
		'bg-(--vp-c-yellow-3)': progress?.puzzleState === 'unlocked',
		'shadow-black': active,
	}]">
		<Lock v-if="!progress || progress.puzzleState === 'locked'" />
